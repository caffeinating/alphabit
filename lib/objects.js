...
export function containsText(o, text) {
    ...
    const result = Object.keys(o).filter(each => typeof o[each] === "string").find(each => o[each].toLowerCase().includes(test));
...
export function find(o, text) {
    const result = Object.keys(o).find(each => o[each].includes(test));
    return result != undefined;
}
